...
export function renderExhibit(exhibit) {
  ...
  let sidebar = renderSidebar(
    'In This Exhibit',
    exhibit.body.filter(block => ( SIDEBAR_TYPES.includes(block.type) && block?.value?.show_sidebar ) ),
    showAuthors,
    showFootnotes
  )
  ...
  if (showAuthors) {
    ...
    let bubbles
    if (exhibit.authors) {
      bubbles = exhibit.authors.map((author, index) =>
        renderAuthorBubble(author, 'stack', index)
      )
    }
    exhibitAuthor = (
